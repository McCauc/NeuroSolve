...
class MainContentFrame(ctk.CTkFrame):
    def __init__(self, master, export_callback=None, **kwargs):
        ...
        Tooltip(self.dl_btn, "Export the current solution trail as TXT, HTML, or PDF")
    ...
    def set_export_enabled(self, enabled: bool):
        ...
        self.dl_btn.configure(state="normal" if self._export_enabled else "disabled")

    def _handle_download(self):
        if not self._export_enabled:
            self.log_export_status("Export is unavailable until a completed solve or handled warning exists.", style="warning")
            return
        if callable(self.export_callback):
            self.export_callback()

    def log_export_status(self, message: str, style: str = "normal"):
        self._append_log(message, title="EXPORT", style=style)
    ...
    def _render_verification_step(
        ...
        self.add_step("VERIFICATION", format_verification_block(payload), style=style)

    def update_success(self, root: float, iterations: int, verification: Optional[Dict[str, Any]] = None):
        self.add_step("FINAL", f"Root bounded at x = {root:.8f}", style="success")
        self._render_verification_step(
            verification=verification,
            fallback_status="success",
            fallback_summary="Solver converged but verification details were unavailable.",
            style="success",
        )
    ...
    def update_error(
        ...
        if classification == "warning":
            ...
            self._render_verification_step(
                ...
                fallback_summary="No trustworthy estimate exists for this run.",
                style="warning",
            )
        ...
        if isinstance(verification, dict):
            self._render_verification_step(
                ...
                fallback_summary="Verification payload received during error handling.",
                style="error",
            )
